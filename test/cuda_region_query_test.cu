...
TEST(CudaRegionQueryTest, basic_query) {
    ...
    vector_device<atom_t<double>> d_atoms(num_atoms, stream);
    cudaMemcpyAsync(d_atoms.data(), atoms.data(), num_atoms * sizeof(atom_t<double>), cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);

    unit_cell.set_atoms(std::move(atoms));

    // Test query around a point
    vector<point3d_t<double>> query_points = {
    ...
    double cutoff = dist(gen);
    std::cout << "cutoff: " << cutoff << std::endl;

    vector_device<point3d_t<double>> d_query_points(query_points.size(), stream);
    cudaMemcpyAsync(d_query_points.data(), query_points.data(), query_points.size() * sizeof(point3d_t<double>), cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
    ...
    // query
    std::vector<std::vector<query_result_t<double>>> cpu_results;
    for (const auto& query_point : query_points) {
        std::vector<query_result_t<double>> results = region_query.query(query_point, cutoff, brt.get(), &unit_cell);
    ...
    cuda_region_query_t<uint32_t, int32_t, double> d_region_query(
        region_query.get_unique_morton_codes(), num_bits_per_dim, 
        region_query.get_offsets(), region_query.get_sorted_indexes());

    vector_device<cuda_query_result_t<double>> query_results(1, stream);
    ...
    vector<cuda_query_result_t<double>> h_query_results(query_results.size());
    cudaMemcpyAsync(h_query_results.data(), query_results.data(), query_results.size() * sizeof(cuda_query_result_t<double>), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    ...
    for (auto i = 0; i < query_points.size(); i++) {
        ...
        for (auto j = 0; j < cpu_results[i].size(); j++) {
            EXPECT_DOUBLE_EQ(h_query_results[gpu_offset + j].distance_squared, cpu_results[i][j].distance_squared);
